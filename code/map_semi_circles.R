...
affs <- reshape2::melt(aff_data %>% dplyr::select(-pubyr, -author1last, -author2last, 
                                          -reftitle, -doi, -publication_type, -complete_aff, -grid_code, -journal, -ethics), 
...
aff.plot$start <- rep(c(0, pi), nrow(affs)/2)

r <- 15 #radius of biggest circle
scale <- r/max(sqrt(aff.plot$count))

theme_set(theme_void(base_size=14) %+replace%
...
geom_polygon(data=world_map, aes(x = long, y = lat, group = group), 
  ...
  geom_polygon(data=world_map[world_map$region == "Myanmar",], aes(x = long, y = lat, group = group), 
               fill="#dd9c69ff", colour = "#d07120ff", size=0.5) +
  ...
  geom_text(data = aff.plot[aff.plot$count > 20 & duplicated(aff.plot$aff_country)== FALSE,],
            aes(label = aff_country, x = long, y = lat + scale*sqrt(count) + .05),
            size =11/.pt, vjust = 0, hjust=-0.2, family="Roboto", fontface=2) +
