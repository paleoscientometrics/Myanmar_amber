...
url <- "https://search.sciencemag.org/?searchTerm=myanmar%20amber&order=tfidf&limit=textFields&pageSize=100&startDate=2015-01-01&endDate=2020-11-21&articleTypes=Research%20and%20reviews&"
...
alt_mets <- alt_mets[lapply(alt_mets,length)>0]

#write.csv(res, file.path("output", "nature_all_paleo.csv"), row.names = FALSE)
openxlsx::write.xlsx(cbind(id=paste0("sci", 1:nrow(res)), res),file.path("output", "science_all_myanmar.xlsx") ) #encoding purposes
write.csv(data.table::rbindlist(alt_mets, fill=TRUE), file.path("output", "alt_science_all_myanmar.csv"), row.names = FALSE)
